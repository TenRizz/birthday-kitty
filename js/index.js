/**
 * 💖 Birthday Surprise — Ultra Premium "Rose Pink & Starlight Gold" JS 💖
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    class BGParticle {
        constructor() {
            ...
            this.color = ['#ffd6eb', '#ff80b3', '#ffe6f0', '#ff4d94'][Math.floor(Math.random() * 4)];
        }
    ...
    const triggerBurst = (x, y, count = 30, type = 'spark') => {
        const colors = ['#ff4d94', '#ff80b3', '#ffd6eb', '#ffeb99', '#ffffff'];
    ...
    const crackSeal = (e) => {
        ...
        envelopeHint.textContent = "Opened! Tap the button on the letter... ✨";
    ...
    const initPolaroidStack = () => {
        ...
        const media = [
            { type: 'image', src: 'assets/foto1.png', label: '🩷' },
        ];
    ...
    const finishCut = (clientX, clientY) => {
        ...
        setTimeout(() => {
            cakeTitle.textContent = 'Yummy! Make a Wish...';
    ...
    const ytPlayerEl  = document.getElementById('yt-music-player');
    const ytVideoId   = ytPlayerEl?.dataset.ytId || '450p7goxZqg';

    // Shared player state
    window._ytReady     = false;   // true once YT API + player are ready
    window._ytPlayer    = null;    // YT.Player instance
    window._ytPlayQueued = false;  // play was requested before player ready
    window._ytMuted     = false;   // current mute state

    // Called by YouTube IFrame API when script is loaded
        ...
        window._ytPlayer = new YT.Player('yt-iframe', {
            ...
            events: {
                onReady: (e) => {
                    ...
                    if (window._ytPlayQueued) {
                        e.target.playVideo();
                        musicToggle?.classList.add('playing', 'visible');
                        if (musicToggle) musicToggle.textContent = '🎵';
                        window._ytPlayQueued = false;
                    }
                },
                onError: (e) => {
                    console.warn('[YT Music] Player error:', e.data);
                }
            }
        });
    };
    ...
    const playMusic = () => {
        if (!window._ytReady) {
            // Queue play — will fire in onReady
            window._ytPlayQueued = true;
            return;
        }
        if (window._ytPlayer?.getPlayerState() !== YT.PlayerState.PLAYING) {
    ...
    const wishOverlay = document.getElementById('wish-popup-overlay');
    const wishText = document.getElementById('wish-popup-text');
    const wishTitle = document.getElementById('wish-popup-title');
    const wishClose = document.getElementById('wish-popup-close');
    const wishBtn = document.getElementById('wish-popup-btn');
    let popupTimer = null;

    const closeWishPopup = () => {
        clearTimeout(popupTimer);
        if (wishPopup) wishPopup.classList.remove('show');
        if (wishOverlay) wishOverlay.classList.remove('show');
    };
    ...
    document.querySelectorAll('.wish-bubble').forEach(bubble => {
        const popBubble = (e) => {
            ...
            triggerBurst(bRect.left + bRect.width / 2, bRect.top + bRect.height / 2, 28, 'heart');

            // Extra burst of sparkles at screen center
            triggerBurst(window.innerWidth / 2, window.innerHeight / 2, 18, 'star');
            ...
            // Set dynamic title and text message
            const title = bubble.dataset.title || bubble.textContent.trim() || 'A Special Wish';
            const msg = bubble.dataset.msg || '';

            if (wishTitle) wishTitle.textContent = title;
            if (wishText) wishText.textContent = msg;

            if (wishPopup) wishPopup.classList.add('show');
            if (wishOverlay) wishOverlay.classList.add('show');
    ...
    if (wishClose) {
        wishClose.addEventListener('click', closeWishPopup);
    }
    if (wishBtn) {
        wishBtn.addEventListener('click', closeWishPopup);
    }
    if (wishOverlay) {
        wishOverlay.addEventListener('click', closeWishPopup);
    }

    // Dismiss popup on backdrop tap
    wishPopup.addEventListener('click', (e) => {
        if (e.target === wishPopup) {
            closeWishPopup();
        }
    });
    wishPopup.addEventListener('touchstart', (e) => {
        if (e.target === wishPopup) {
            closeWishPopup();
        }
    }, { passive: true });
